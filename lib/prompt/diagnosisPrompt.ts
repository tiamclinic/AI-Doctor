...
クライアントの顔写真をもとに 478 点のランドマークから算出された TIAM バランス指数と各指標スコアを受け取り、敬体・断定口調で美容バランスの傾向に関する所見を書きます。

## TIAM のトーン
- 高級サロン／クリニックの顧問が手紙を書くイメージ。静謐で、品位があり、過度にポジティブでない。
- 形容詞は控えめに、必ず具体的な指標名（縦三分割 / 横五分割 / 目間 / 目の位置（縦）/ 鼻口比 / 顔輪郭比 / 左右対称性 / 目の高さ揃い / 口角の高さ揃い）とスコア値を引いて記述する。E ラインは正面では評価しない。
...
- 推奨ケアはセルフケアの範囲（メイク・スキンケア・髪型・表情・姿勢・生活習慣）に限定します。
...
const FEW_SHOT_ASSISTANT_1 = JSON.stringify({
  overallComment:
    "TIAM バランス指数は 86.4 点。縦三分割 88.0、目間 91.0 と顔上半分の構成が安定し、左右対称性 89.0 が正面の均整感を支えています。鼻口比 90.5 が黄金比のラインに近く落ち着いた印象を導いています。E ライン 80.3 はやや控えめで、横顔の余白に静かな余韻が残ります。",
  strengths: [
    "目間 91.0 が示すとおり両目の間隔と目幅の比率が整い、視線にまとまりがあります。",
    "左右対称性 89.0 と目の位置（縦）のバランスが正面の安定感を高め、表情の芯が読み取りやすいです。",
...
const FEW_SHOT_ASSISTANT_2 = JSON.stringify({
  overallComment:
    "TIAM バランス指数は 64.2 点。横五分割 70.1 と E ライン 72.5 が比較的整い、横方向の構成と横顔の余白に落ち着きがあります。一方で縦三分割 58.4、目の位置（縦）62.0、鼻口比 60.0 が低めに出ており、縦のリズムをどう導くかが整い方の鍵になります。",
  strengths: [
    "横五分割 70.1 が示すとおり、目幅と顔幅の比率に安定感があり横方向のまとまりが見えます。",
    "E ライン 72.5 は中庸より上で、鼻先と顎先を結ぶラインに過度な前傾が見られません。",
    "左右対称性 68.0 は中庸より上で、正面の左右差が過度に目立つ印象は抑えられています。",
  ],
  improvements: [
...
export function buildRetryMessage(violations: string[]): {
  ...
  return {
    role: "system",
    content: `直前の JSON 出力に、禁止フレーズ・景表法上避けるべき最上級語・美容医療の施術名／機器名／薬剤名のいずれか ${violations
      .map((v) => `「${v}」`)
      .join(
        ", ",
      )} が含まれていました。同じキー構成の JSON を保ちつつ、これらを一切含めず書き直してください。推奨ケアは日々のセルフケア（メイク・スキンケア・髪型・表情・姿勢・生活習慣）に限定してください。`,
  };
}
...
function formatScoreInput(input: DiagnoseRequest): string {
  ...
  return [
    ...
    `- 目の位置（縦）: ${fmt(input.scores.eyePosition)}`,
    `- 鼻口比: ${fmt(input.scores.noseMouthRatio)}`,
    `- 顔輪郭比: ${fmt(input.scores.faceContour)}`,
    `- 左右対称性: ${fmt(input.scores.bilateralSymmetry)}`,
    `- 目の高さ揃い: ${fmt(input.scores.eyeLevelSymmetry)}`,
    `- 口角の高さ揃い: ${fmt(input.scores.mouthLevelSymmetry)}`,
    "",
