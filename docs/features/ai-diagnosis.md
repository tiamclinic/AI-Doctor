...
| [`lib/prompt/forbiddenWords.ts`](../../lib/prompt/forbiddenWords.ts) | 禁止フレーズ / 医療表現置換辞書 |
...
3. **`scanForbidden()`** で残存禁止フレーズを検出（「いかがでしょうか」「最も美しい」など）
4. 検出されたら **1 回だけリトライ**: 直前の出力を assistant メッセージとして渡し、`buildRetryMessage()` で書き直しを system に指示
5. リトライも失敗したら、置換済みの 1 回目を返す（落としきれない場合のフォールバック）
