# 03. 黄金比スコアリング（TIAM 指標）

| 項目       | 内容                                |
| ---------- | ----------------------------------- |
| チケットID | T-03（拡張: [T-18](./18-eye-position-symmetry-metrics.md)）        |
| 関連要件   | F-03                                |
| 依存       | T-02                                |
| 優先度     | 高                                  |
| 見積       | 3 日                                |
...
478 点のランドマークから **TIAM 指標**（現状 8 項目）を計算し、各指標を 0〜100 点に正規化、加重平均で `TIAM バランス指数` を算出する。クライアント側で完結させる。

## ゴール / 受け入れ基準

- [x] 各指標が計算され、それぞれ 0〜100 点で出力される
...
- [x] ユニットテスト（最低限のスナップショット）が用意されている（Vitest）
...
| 目間バランス       | 両目間距離 / 目幅                            | 1.0       |
| 目の位置（縦）     | 両眼 y 平均が額上端〜顎の高さに占める比率    | 0.5（院内・ダミー整合） |
| 鼻口比率           | 鼻幅 / 口幅                                  | 1 : 1.618 |
| E ライン整合度     | 鼻先・上唇・下唇・顎先の直線関係             | on-line   |
| 顔輪郭比率         | 顔幅 / 顔長                                  | 1 : 1.46  |
| 左右対称性         | 左右対応点ペアの中点 x が頬中点からずれる量の平均（顔幅正規化） | 0 |
...
実値は `lib/faceAnalysis/scoring.ts` の `METRIC_WEIGHTS` を参照（T-18 追加後も合計 1.0）。
...
- [x] `lib/faceAnalysis/goldenRatio.ts` で各指標の生値計算関数を実装
- [x] `lib/faceAnalysis/scoring.ts` で正規化 + 加重平均を実装
- [x] 重み・係数 `k` のチューニングメモを `scoring.ts` 末尾に記載
- [x] `__tests__/goldenRatio.test.ts` を Vitest で実装（理想値 / 局所崩れ / 左右非対称 / 下限ガード / 決定論 / 例外）
