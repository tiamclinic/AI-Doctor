...
| `GET` / `PUT` | `/api/doctor-notes/{resultId}` | 診断単位のドクター所見 | PUT は staff/admin | [doctor-notes.md](./doctor-notes.md) |
| `GET`  | `/api/diagnoses` | 診断一覧（管理用） | staff/admin | [diagnoses.md](./diagnoses.md) |

> 廃止: `GET /api/doctor-content`（共通テンプレ）は [doctor-content.md](./doctor-content.md) を参照。
