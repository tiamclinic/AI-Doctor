...
|------------|------|------|
| `limit`    | 30   | 1〜100 |
| `cursor`   | —    | 前ページ最終行の `createdAt`（ISO 8601）。`startAfter` 用 |

### レスポンス

- `200` — `{ "items": DiagnosisListItem[], "nextCursor": string | null }`
  - 各 item は `DiagnosisRecord` に `noteStatus`（`none` / `draft` / `published`）を付与
  - `doctor_notes/{resultId}` をサーバー側で結合
  - `nextCursor` が非 null のとき、続きは `?cursor=<nextCursor>&limit=50` 等で取得
- `401` / `403` — 認可エラー
- `503` — Firebase 未設定
