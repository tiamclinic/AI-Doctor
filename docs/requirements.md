...
| F-09 | ドクター個別ノート | 診断 ID ごとに、医師がパーツ別の施術・方針コメントを記入し、結果画面に併用表示する | 必須   |

> **F-09 の段階的実装**: 既に T-13/T-14 でクリニック共通のテンプレ層（`doctor_contents/default`）を実装済み。これは免責・共通注意書きのテンプレートとして残し、**診断単位の個別ノート層**を追加する形で F-09 を完成させる。詳細は §4.9.2 / §4.10。
...
- **US-06**: 医師（または院スタッフ）として、来院者の AI 診断結果を管理画面で開き、AI スコア・パーツ分析を見ながら **その人向けの施術・ケア方針** をパーツ別に記入・公開したい。
- **US-07**: 医師として、テンプレ的な共通注意書き（免責・院方共通メッセージ）と、診断単位の個別ノートを **別の階層** で管理したい（共通テンプレを再利用しつつ、個別の追記ができる）。
- **US-08**: 医師として、過去の診断一覧から目的の診断 ID を検索し、当該診断の元データ（撮影日時・スコア・サムネイル等）を確認したうえで記入したい。
...
flowchart TD
    ...
    H --> I[結果画面表示<br/>AI 由来コメント + 共通テンプレ表示]
    I --> J[シェアカード ダウンロード/SNS シェア]
    I -.->|医師が後から個別記入<br/>院内・別タイミング| K[管理画面: 診断一覧]
    K --> L[診断 ID を選択<br/>AI スコア・パーツ分析を確認]
    L --> M[パーツ別ノート記入<br/>共通テンプレ + 個別追記]
    M --> N[公開: doctor_notes/{resultId}]
    N -.->|来院者が再閲覧| I
```

> 医師フロー（破線部分）は **別タイミング・別端末** で発生し得る。来院者が結果 URL を再訪したとき、または院内端末で医師と一緒に閲覧したときに、**当該診断 ID に紐付いた個別ノートが追加で表示される**。
...
#### 4.9.2 ドクター記述データの構造（2 層構造：共通テンプレ ＋ 個別ノート）

ドクター記述は、目的が異なる **2 つの階層** で構成する。

##### 4.9.2.1 共通テンプレ層（クリニック 1 拠点 = ドキュメント 1 本）

- 既に T-13 / T-14 で実装済み（Firestore: `doctor_contents/default`）
- 用途: **冒頭メッセージ・免責文・パーツ別の一般的注意書き** などの**全結果共通テンプレート**
- 編集 UI: `/admin/doctor-content`（パーツタブ + プレビュー + 公開）
- 表示位置: 結果画面の **共通枠**（または個別ノートが空のときのフォールバック）
- 単体では「あなた向け」のメッセージ性は持たない（依頼者要望を満たせない）

##### 4.9.2.2 診断個別ノート層（診断 ID ごとに 1 件、本サービスの中核）
...
- 表示位置: 結果画面の **各パーツカード内**、AI 由来コメントの直下に **「当院医師より（{担当者名}）」**として並べる
- 共通テンプレ層との関係: **個別ノートが存在すれば優先表示**、無ければ共通テンプレを表示、両方無ければ AI セクションのみ
- 各パーツには **見出し（任意）・本文（箇条書き可）・推奨ケア・所感** を紐付けられる
- **記入者（医師の表示名）・記入日時・公開ステータス（下書き / 公開）** を保持する

#### 4.9.3 併用表示と印刷

- 同一の結果画面（または印刷用ビュー）上で、**数値スコア＋AI 診断レポート＋共通テンプレ＋個別ノート**を一連のレポートとして閲覧できること。
- 個別ノートが存在する場合は、AI 由来ブロックの直後に **「当院医師より」** バッジ付きでパーツごとに表示する。
- 個別ノート未公開の場合は、共通テンプレを下位フォールバックとして用い、視覚的には「TIAM ガイドライン」「院方共通注意事項」など**一般情報**である旨を明示する。
...
1. 来院者が通常フローで AI 黄金比診断を実行（写真は従来どおりクライアント解析のみ）
2. **診断結果のメタデータ（スコア・指標・サムネイル・diagnosisText）が `diagnoses/{resultId}` として Firestore に保存される**（個別ノートを後から書くための前提）
   - 写真本体は依頼者の運用ポリシーに応じて、サムネイル（解像度を落とした顔写真）のみを保持する案を初期採用とする
   - 保存対象を細分化する設計（写真なし／サムネイルのみ／フル）は実装時に決定可能
3. 医師（または院スタッフ）が `/admin/diagnoses` を開き、診断一覧から目的の `resultId` を選択
4. 個別編集画面で AI スコア・パーツ分析・診断テキストを見ながら、**パーツ別に個別ノートを記入** → プレビュー → 公開
5. 来院者が結果 URL を再訪したとき、または院内端末で結果画面を開いたとき、**`doctor_notes/{resultId}` を取得して併用表示**
...
#### 4.9.6 ドクター管理画面の要件（F-09 の UI 詳細）

- `/admin/diagnoses` … 診断一覧（撮影日時・スコア・サムネイル・記入状況バッジ・絞り込み: 「未記入のみ」など）
- `/admin/diagnoses/{resultId}` … 個別編集画面
  - AI スコア・パーツ分析・AI 診断テキストを **読み取り専用** で並べる
  - 右側（または下側）にパーツ別エディタ：本文・推奨ケア・内部メモ
  - **共通テンプレを「下書きにコピー」**するボタン（テンプレを起点に微修正できる）
  - プレビュー（実際の結果画面と同じカードレイアウト）
  - 「下書き保存」「公開」「公開取り消し」
- 認証: 既存の Firebase Auth + `admin` カスタムクレーム（T-14 と統合）
- API: `GET/PUT /api/doctor-notes/{resultId}`（admin のみ書き込み / 読み取りは結果画面からも許可）

#### 4.9.7 個別ノートの受入基準

- [ ] 通常フローで AI 診断を実行すると、診断結果のメタデータが Firestore `diagnoses/{resultId}` に保存される
- [ ] `/admin/diagnoses` に少なくとも直近 30 件の診断一覧が出る（ページング・絞り込みは段階導入可）
- [ ] `/admin/diagnoses/{resultId}` で、当該診断の AI スコア・パーツ分析を見ながらパーツ別ノートを記入・公開できる
- [ ] 公開されたノートは、来院者の結果画面 `/result/{resultId}` を再訪すると **AI 由来コメントの直下に「当院医師より」バッジ付きで表示**される
- [ ] 共通テンプレ層と個別ノート層が **同時に存在する場合は個別ノートを優先表示** し、視覚的に出典を区別する
- [ ] 公開ノートは **記入者と公開日時** を結果画面に併記する
- [ ] `npm run lint` / `npm run build` / 既存テストがクリーン

### 4.10 F-09 ドクター個別ノート（管理画面 + 結果画面連携）

依頼者要望（§1.5 / §4.9）に基づく **本サービスの中核機能**。AI が踏み込まない施術・ケア方針を、診断 ID 単位で医師が直接記述するためのスタックを提供する。

- **対象データ**: 診断結果（`diagnoses/{resultId}`）と個別ノート（`doctor_notes/{resultId}`）
- **登場アクター**: 医師 / 院スタッフ（admin クレーム）、来院者（結果 URL）
- **エンドポイント**:
  - `POST /api/diagnoses`（暗黙）— 既存の診断フロー完了時に診断メタデータを保存
  - `GET  /api/diagnoses`（admin）— 一覧
  - `GET  /api/diagnoses/{resultId}`（admin / 来院者）— 詳細
  - `GET  /api/doctor-notes/{resultId}` — 個別ノート読み取り（読み取り公開可）
  - `PUT  /api/doctor-notes/{resultId}`（admin）— 個別ノート公開
- **依存タスク**: T-13 / T-14（共通テンプレ層）、T-15（結果画面の併用表示）
- **新規タスク（チケット名は実装時に確定）**:
  - 診断結果の永続化（`diagnoses/{resultId}`）
  - 管理画面 `/admin/diagnoses`（一覧）
  - 管理画面 `/admin/diagnoses/{resultId}`（個別編集）
  - `doctor_notes` API（GET / PUT）と Firestore セキュリティルール
  - 結果画面の併用表示（個別ノート優先 + 共通テンプレフォールバック）
- **詳細**: §4.9.5〜§4.9.7
...
- F-09 ドクター個別ノートの前提として保存するのは **診断メタデータ（スコア・指標・診断テキスト）と任意のサムネイル** に限定し、写真原本の保存有無は院の運用ポリシーで選択可能とする
...
- [ ] **F-09**: 通常フローで実行した診断が管理画面の診断一覧に表示され、医師が当該診断を開いて個別ノートを記入・公開できる
- [ ] **F-09**: 公開された個別ノートが、対応する結果 URL の再訪時に「当院医師より（記入者名 / 日時）」付きで AI コメントと併用表示される
- [ ] **F-09**: 共通テンプレ層と個別ノート層がある場合、個別ノートが優先表示され、視覚的に出典が区別される
